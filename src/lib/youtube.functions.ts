...
import { getYouTubeChannelByHandle, getYouTubeChannelSnapshot, getYouTubeChannelVideos, getYouTubeVideoSnapshot } from "./youtube.server";

export const analyzeYouTubeChannelByHandle = createServerFn({ method: "POST" })
  .validator(z.object({ handle: z.string().min(2).max(100), limit: z.number().int().min(1).max(24).default(12) }))
  .handler(async ({ data }) => {
