...
function ScriptsPage(){
 ...
 useEffect(()=>{try{const raw=localStorage.getItem("viralflow.savedIdeas");if(raw)setIdeas(JSON.parse(raw)); const reportRaw=localStorage.getItem("viralflow.learningReport"); if(reportRaw){const report=JSON.parse(reportRaw) as Partial<typeof learningContext>; setLearningContext({strongestPatterns:Array.isArray(report.strongestPatterns)?report.strongestPatterns:[],weakPatterns:Array.isArray(report.weakPatterns)?report.weakPatterns:[],experimentsToRun:Array.isArray(report.experimentsToRun)?report.experimentsToRun:[]});}}catch{}},[]);
 async function create(){
  ...
  try{const r=await generateScript({data:{concept:selected.concept,angle:selected.angle,promise:selected.promise,audience:selected.audience,format:selected.format,hook:hook.trim(),durationSeconds:duration,learningContext,route:"balanced"}});if(!r.parsed){setMessage("O motor não devolveu um roteiro utilizável.");return}setScript(r.parsed); localStorage.setItem("viralflow.lastScript", JSON.stringify({...r.parsed, format:selected.format}))}
